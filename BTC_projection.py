...
strl.caption("Assuming Bitcoin continues to be adopted over time this chart can be used as a price prediction tool where there is potential for price movement between the upper and lower boundaies as support and resistance. These levels are estimated according to the price deviations from the logarithmic growth trend of BTC across all its history. The price levels corresponding to each risk level can be seen by hovering over the graph, and can be understood as the price deviation uncertainity from the long term growth trend. As a hint, the best buying points happened below the 20% risk level and peaks have always occurred above the 90% level. Because the curves can be calculated on a forward-looking basis, it is possible to forecast where the price of Bitcoin may move towards in the future. Have fun speculating")
