...
# 7. Footer
strl.write("------------------------------------------------------")
strl.markdown("Assuming Bitcoin continues to be adopted over time this chart can be used as a price prediction tool where there is potential for there to be resistance on the upper boundary and support on the lower boundary which are estimated according to the logarithmic price deviation over all BTC's time. The price risk levels of those boundaries can be seen by hovering over them and can be understood as the price deviation uncertainity from the long term growth. As a hint, the best buying points happened below the 20% risk level and peak have occurred always above the 90% level. Because the curves can be calculated on a forward-looking basis, it is possible to forecast where the price of Bitcoin may move towards in the future.")
